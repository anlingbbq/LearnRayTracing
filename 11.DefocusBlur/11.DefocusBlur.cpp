// 11.DefocusBlur.cpp : This file contains the 'main' function. Program execution begins and ends there.
//

#include "PPMRender.h"

class DefocusBlurRender : public PPMRender
{
	void init() override
	{
		nx = 1280;
		...
		vec3 lookform(-2, 2, 1);
		vec3 lookat(0, 0, -1);
		float dist_to_focuse = (lookform - lookat).length();
		float aperture = 2.0f;
		cam = new camera(lookform, lookat, vec3(0, 1, 0), 90, float(nx / ny), aperture, dist_to_focuse);
...
int main()
{
	DefocusBlurRender dr;
	dr.output("DefocusBlurRender");
}
